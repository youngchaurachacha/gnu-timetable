...
def load_and_process_data(file_path, major_sheet, general_sheet):
    """
    원본 엑셀 파일에서 데이터를 읽고, 수업방식 정보를 포함하여 처리한다.
    """
    try:
    ...
    # --- 여기가 수정됨: '수업방법' 컬럼 추가 ---
    general_cols = ['교과목명', '교수명', '학점', '이수구분', '학과', '수강반번호', '강의시간/강의실', '캠퍼스구분', '교과목코드', '수업방법']
    ...
    df_combined['대상학년'] = df_combined['대상학년'].fillna('')
...
if master_df is not None:
    ...
    with tab_major:
        ...
        if selected_depts:
            ...
            # --- 여기가 수정됨: 수업방법 정보 추가 ---
            course_options = filtered_df.apply(lambda x: f"[{x['대상학년']}/{x['이수구분']}/{x['수업방법']}] {x['교과목명']} ({x['교수명']}, {x['분반']}반)", axis=1).tolist()
            ...
            if not course_options:
                ...
                if st.button("전공 추가", key="add_major"):
                    selected_row = filtered_df[filtered_df.apply(lambda x: f"[{x['대상학년']}/{x['이수구분']}/{x['수업방법']}] {x['교과목명']} ({x['교수명']}, {x['분반']}반)", axis=1) == selected_course_str].iloc[0]
                    st.session_state.my_courses.append((selected_row['교과목코드'], selected_row['분반']))
    ...
    with tab_general:
        ...
        selected_cat = st.selectbox("교양 이수구분을 선택하세요.", categories)

        if selected_cat:
            filtered_gen_df = general_df[general_df['이수구분'] == selected_cat]
            # --- 여기가 수정됨: 수업방법 정보 추가 ---
            course_options_gen = filtered_gen_df.apply(lambda x: f"[{x['수업방법']}] {x['교과목명']} ({x['교수명']}, {x['분반']}반, {x['학점']}학점)", axis=1).tolist()

            if not course_options_gen:
                st.warning("해당 이수구분에 현재 추가 가능한 교양 과목이 없습니다.")
            else:
                ...
                if st.button("교양 추가", key="add_general"):
                    selected_row = filtered_gen_df[filtered_gen_df.apply(lambda x: f"[{x['수업방법']}] {x['교과목명']} ({x['교수명']}, {x['분반']}반, {x['학점']}학점)", axis=1) == selected_course_str_gen].iloc[0]
                    st.session_state.my_courses.append((selected_row['교과목코드'], selected_row['분반']))
                    st.success(f"'{selected_row['교과목명']}' 과목을 추가했습니다.")
                    st.rerun()
    ...
    if not st.session_state.my_courses:
        ...
        days = ['월', '화', '수', '목', '금', '토']
        html = """<style>.timetable { width: 100%; border-collapse: collapse; }.timetable th, .timetable td { border: 1px solid #e6e6e6; text-align: center; vertical-align: middle; padding: 4px; }.timetable th { background-color: #f0f2f6; }.row-3 { height: 2.5em; }</style><table class="timetable"><tr><th>교시</th><th>시간</th><th>월</th><th>화</th><th>수</th><th>목</th><th>금</th><th>토</th></tr>"""
        timetable_data = { (p, d): ["", "", ""] for p in range(1, 13) for d in days }
                ...
                for time_info in course['parsed_time']:
                    for p in time_info['periods']:
                        if time_info['day'] in days and p in range(1, 13):
                            timetable_data[(p, time_info['day'])] = [course['교과목명'], course['교수명'], time_info['room']]
            else:
                untimed_courses.append(course)
        time_map = {p: f"{p+8:02d}:00~{p+8:02d}:50" for p in range(1, 13)}
        for p in range(1, 13):
            html += f'<tr><td rowspan="3" class="row-3">{p}</td><td rowspan="3" class="row-3">{time_map[p]}</td>'
            for d in days: html += f'<td>{timetable_data.get((p, d), [""])[0]}</td>'
            html += '</tr><tr>'
            for d in days: html += f'<td>{timetable_data.get((p, d), [""])[1]}</td>'
            html += '</tr><tr>'
            for d in days: html += f'<td>{timetable_data.get((p, d), [""])[2]}</td>'
            html += '</tr>'
        html += "</table>"
